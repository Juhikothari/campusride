...
// Maps common abbreviations/variations to a canonical college name
// so "rns", "rnsit", "r.n.s" all resolve to "RNSIT"
...
function normalizeCollege(name) {
  if (!name) return '';
  const lower = name.trim().toLowerCase().replace(/[.\-_]/g, ' ').replace(/\s+/g, ' ');
  return COLLEGE_ALIASES[lower] || lower;
}

module.exports = { COLLEGE_DOMAINS, getDomainsForCollege, validateCollegeEmail, normalizeCollege };

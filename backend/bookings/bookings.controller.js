...
exports.getMyBookings = async (req, res) => {
  try {
    const bookings = await Booking.find({ seekerId: req.user.userId })
      .populate('rideId', 'pickup drop date time costPerSeat status vehicleType').populate('seekerId', 'name phone usn gender')
      .sort({ createdAt: -1 });
...
exports.getRideRequests = async (req, res) => {
  try {
    ...
    .populate('seekerId', 'name phone rating usn gender')
    .sort({ createdAt: -1 });

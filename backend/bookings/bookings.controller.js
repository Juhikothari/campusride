...
exports.requestBooking = async (req, res) => {
  try {
    ...
    const ride = await Ride.findById(rideId).populate('providerId', 'name fcmToken socketId college kycDocuments usn');
    if (!ride) {
...
exports.respondBooking = async (req, res) => {
  try {
    ...
    // If accepted: cancel ALL other active rides from same provider
    if (status === 'accepted') {
      try {
        const acceptedRide = booking.rideId;
        const rideDate = new Date(acceptedRide.date);
        const dayStart = new Date(rideDate); dayStart.setHours(0,0,0,0);
        const dayEnd   = new Date(rideDate); dayEnd.setHours(23,59,59,999);

        const otherRides = await Ride.find({
          providerId: acceptedRide.providerId,
          _id: { $ne: acceptedRide._id },
          status: 'active',
          date: { $gte: dayStart, $lte: dayEnd },
        });

        console.log(`Auto-cancelling ${otherRides.length} other rides for provider`);

        const io = req.app.get('io');
        for (const otherRide of otherRides) {
          otherRide.status = 'cancelled';
          otherRide.cancelReason = 'Provider already accepted another ride for this time';
          otherRide.cancelledAt = new Date();
          await otherRide.save();

          const pendingBookings = await Booking.find({ rideId: otherRide._id, status: { $in: ['pending','accepted'] } });
          for (const ob of pendingBookings) {
            ob.status = 'cancelled';
            await ob.save();
            if (io) {
              io.to(`user-${ob.seekerId}`).emit('booking-response', {
                booking: { _id: ob._id, status: 'cancelled' },
                notification: { title: '❌ Ride Cancelled', body: 'The provider accepted another booking. Please search for a new ride.' }
              });
            }
          }
        }
...
// ── Seeker cancels their own booking ────────────────────────────
exports.cancelBooking = async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    if (io && booking.rideId?.providerId) {
      io.to(`user-${booking.rideId.providerId}`).emit('booking-cancelled', {
        bookingId: booking._id,

...
exports.respondBooking = async (req, res) => {
  try {
    ...
    // If accepted: cancel all OTHER active rides from the same provider for the same time slot
    if (status === 'accepted') {
      try {
        const acceptedRide = booking.rideId;
        // Find other active rides by same provider on the same date
        const otherRides = await Ride.find({
          providerId: acceptedRide.providerId,
          _id: { $ne: acceptedRide._id },
          status: 'active',
          date: acceptedRide.date,
        });

        for (const otherRide of otherRides) {
          otherRide.status = 'cancelled';
          otherRide.cancelReason = 'Provider accepted another ride at this time';
          otherRide.cancelledAt = new Date();
          await otherRide.save();

          // Cancel pending bookings on those rides and notify seekers
          const otherBookings = await Booking.find({ rideId: otherRide._id, status: 'pending' });
          for (const ob of otherBookings) {
            ...
            if (io) {
              io.to(`user-${ob.seekerId}`).emit('booking-response', {
                ...
                notification: { title: 'Ride Cancelled', body: 'The provider accepted another ride. Please search for another ride.' }
              });
            }
          }
        }

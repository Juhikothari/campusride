/**
 * NearbyMap.jsx — Fixed:
 *   • Male users do NOT see female provider markers or women-only rides
 */
...
function makeIcon(svg, size=[32,42], anchor=[16,42]) {
  return L.divIcon({ html:svg, className:'', iconSize:size, iconAnchor:anchor, popupAnchor:[0,-40] });
}
...
export default function NearbyMap({ pickupLat, pickupLng, dropLat, dropLng, userGender, className='' }) {
  ...
  useEffect(() => {
    ...
    script.onload = () => { L = window.L; window._leafletLoaded = true; setLeafletReady(true); };
    document.head.appendChild(script);
  }, []);

  useEffect(() => {
    if (!leafletReady || !mapRef.current || mapObj.current) return;
    const center = (pickupLat && pickupLng) ? [parseFloat(pickupLat), parseFloat(pickupLng)] : [12.9716, 77.5946];
  ...
  const fetchNearby = useCallback(async () => {
    ...
    } catch { setError('Could not load nearby providers.'); }
  ...
  useEffect(() => {
    ...
    if (pickupLat && pickupLng) {
      ...
      L.marker([lat, lng], { icon: makeIcon(YOU_ICON_SVG, [28,28], [14,14]) })
        .bindPopup('<b>📍 Your Pickup</b>').addTo(markersLayer.current);
    }
    ...
    if (routeInfo?.coordinates?.length) {
      const poly = L.polyline(routeInfo.coordinates, { color:'#f5a623', weight:4, opacity:0.85 }).addTo(routeLayer.current);
      poly.bindPopup(`🛣️ ${routeInfo.distanceKm} km · ~${routeInfo.durationMin} min`);
      bounds.push(...routeInfo.coordinates.slice(0, 50));
    }

    // ── Provider markers — hide female markers from male viewers ──
    providers.forEach(p => {
      const isFemaleProvider = p.gender === 'female';
      const isWomenOnlyRide  = p.womenOnly;

      // Male users must not see female providers or women-only rides
      if (viewerIsMale && (isFemaleProvider || isWomenOnlyRide)) return;
    ...
    if (bounds.length > 1) {
      try { mapObj.current.fitBounds(bounds, { padding:[40,40], maxZoom:15 }); } catch {}
  ...
  // Visible counts depend on viewer gender
  const viewerIsMale = userGender === 'male';
  const visibleProviders = viewerIsMale
    ? providers.filter(p => p.gender !== 'female' && !p.womenOnly)
    : providers;
  const femaleCount    = visibleProviders.filter(p => p.gender === 'female').length;
  const womenOnlyCount = visibleProviders.filter(p => p.womenOnly).length;
    ...
    <div className={`nearby-map-wrapper ${className}`}>
      <div className="nearby-map-header">
        ...
        <div className="nearby-map-stats">
          ...
          {!loading && visibleProviders.length === 0 && !error && (
            <span className="nearby-stat muted">No providers found nearby</span>
          )}
          {routeInfo && <span className="nearby-stat route">🛣️ {routeInfo.distanceKm} km · {routeInfo.durationMin} min</span>}
        </div>
      </div>
      ...
      <div className="nearby-map-legend">
        ...
        {!viewerIsMale && <span><span className="legend-dot female-provider"/>Female provider</span>}
        {!viewerIsMale && <span><span className="legend-dot women-only-provider"/>Women-only ride</span>}
        <span><span className="legend-dot drop"/>Drop point</span>
      </div>

      <div ref={mapRef} className="nearby-map-canvas"/>
      <p className="nearby-map-note">Tap a marker for provider details.</p>
    </div>
  );
}

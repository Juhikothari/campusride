...
// Passenger-caused issues (provider reporting about a passenger)
...
// Road/rider incidents (provider reporting something that happened to THEM)
...
export default function IncidentReport({ navigate }) {
  ...
  // Pick the right list based on role and subtype
  ...
  if (success) return (
    <div className="page-wrap fade-up" style={{textAlign:'center',paddingTop:60}}>
      ...
      <p className="text-muted mt-4 text-sm">Admin will review within 24 hours.</p>
  ...
  return (
    <div className="page-wrap fade-up">
      ...
      <p className="text-muted mb-20 text-sm">Report any safety concern or issue with a ride.</p>
      ...
      {tab === 'report' && (
        <form onSubmit={submit}>
          ...
          <div className="card" style={{padding:20,marginBottom:16}}>
            ...
            {rides.length === 0 ? (
              <div style={{color:'#666',fontSize:13,padding:'12px',background:'#0d0f14',borderRadius:8,textAlign:'center'}}>
                No rides found. You need an active or completed ride to report an incident.
              </div>
            ) : (
              <select className="input" value={rideId} onChange={e => setRideId(e.target.value)} required
                ...
                {rides.map(r => <option key={r._id} value={r._id}>{getRideLabel(r)}</option>)}
              </select>
            )}
            {selectedRide && (
          ...
          {/* Provider subtype toggle — passenger vs rider */}
          {isProvider && (
          ...
          <div className="card" style={{padding:20,marginBottom:16}}>
            ...
            <p style={{color:'#888',fontSize:12,marginBottom:14}}>
              {isProvider
                ? providerSubtype === 'rider'
                  ? 'Select the road / rider incident category'
                  : 'Select the passenger incident category'
            ...
            <select className="input" value={type} onChange={e => setType(e.target.value)} required
              style={{width:'100%',padding:'12px 14px',borderRadius:8,background:'#0d0f14',
                border:`1px solid ${type?'#f5a623':'#2a2d35'}`,
                color:type?'#f5a623':'#888',fontSize:14,fontWeight:type?600:400,cursor:'pointer'}}>

  ...
  useEffect(() => {
    ...
    api.getRide(rideId)
      .then(data => {
        ...
        isOwnerRef.current = r?.providerId?._id === uid || r?.providerId === uid;
      })
  ...
  useEffect(() => {
    ...
    socket.on('booking-response', (data) => {
      ...
      if (st === 'accepted') {
        ...
        setPopup({ type:'error', message:'❌ This ride was cancelled by the provider.' });
      }
    });
    socket.on('new-booking', () => {
      if (isOwnerRef.current) {
        setPopup({ type:'info', message:'📩 New booking request received!' });
      }
    });

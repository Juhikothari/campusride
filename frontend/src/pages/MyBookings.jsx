...
export default function MyBookings({ navigate }) {
  ...
  useEffect(() => {
    ...
    // Booking accepted/rejected popup
    newSocket.on('booking-response', (data) => {
      const st = data?.booking?.status || data?.status;
      if (st === 'accepted') {
        setPopup({ type:'success', message:'🎉 Your booking has been accepted! Check details below.' });
        fetchBookings();
      } else if (st === 'rejected') {
        setPopup({ type:'error', message:'❌ Your booking was declined. Try another ride.' });
        fetchBookings();
      }
    });
    ...
    <div className="page-wrap fade-up">
      ...
      {popup && (
        <div style={{
          ...
          background: popup.type==='success' ? '#1a3a1a' : '#3a1a1a',
          border: `2px solid ${popup.type==='success' ? '#4caf50' : '#f44336'}`,
          borderRadius:14, padding:'16px 20px',
          boxShadow:'0 8px 32px rgba(0,0,0,0.5)',
          display:'flex', alignItems:'center', gap:12,
        }}>
          <div style={{flex:1, fontSize:14, color:'#fff'}}>{popup.message}</div>
    ...
    <div className={`bk-card card ${isCancelled ? 'cancelled' : ''}`}>
      ...
      <div className="card-body">
        ...
        <div className="grid-2">
          ...
          <div className="text-dim text-xs">VEHICLE
            <div className="text-muted font-700 mt-4">
              {{motorcycle:'🏍️ Bike',car:'🚗 Car',suv:'🚙 SUV',xuv:'🛻 XUV'}[ride.vehicleType] || '🚗'}
            </div>
          </div>
        </div>
